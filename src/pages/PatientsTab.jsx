...
export default function PatientsTab({ processed, selectedPid, onSelectPid, onAddPatient }) {
  const [showLog, setShowLog] = useState({});

  return (
    <div style={{ animation:'fadeIn .4s ease' }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14 }}>
        <div style={{ fontSize:8, color:C.muted, letterSpacing:2, fontFamily:mono }}>PATIENT ANALYSIS ENGINE — {processed.length} PATIENTS</div>
        <button onClick={onAddPatient} style={{ background:C.accent, color:'#000', border:'none', borderRadius:4,
      ...
      {processed.map(p=>{
        ...
        return (
          <div key={p.patient_id} style={{ ...panelStyle,
            borderColor:isSel?sc+'88':p._surgeFlagged?C.purple+'40':C.border,
            background:p._surgeFlagged?C.purple+'04':C.panel,
            transition:'border-color .2s' }}>
            <div onClick={()=>onSelectPid(isSel?null:p.patient_id)}
              style={{ display:'flex', alignItems:'center', gap:12, cursor:'pointer' }}>
              {/* Score circle */}
              <div style={{ width:48, height:48, borderRadius:'50%', border:`2px solid ${heat}`,
                background:heat+'14', display:'flex', alignItems:'center', justifyContent:'center',
            ...
            {isSel && (
              <div style={{ marginTop:14 }}>
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:10, marginBottom:12 }}>
                  {/* Deviations */}
                  <div style={{ background:'#050d18', borderRadius:6, padding:12 }}>
                    <div style={{ fontSize:8, color:C.muted, letterSpacing:1.2, marginBottom:8, fontFamily:mono }}>DEVIATION INDICES (IMAGE PROTOCOL)</div>
                    {[['HR_dev',p.deviations.HR,'|HR−80|'],['BP_dev',p.deviations.BP,'|sys−120|+|dia−80|'],
                      ['O2_drop',p.deviations.O2,'max(0,0.95−SpO2)'],['Fever',p.deviations.Fever,'max(0,T−37.5)×10'],
                      ['Resp_dev',p.deviations.Resp,'|RR−16|'],['Sugar',p.deviations.Sugar,'0 or |S−100|'],
                      ['Age_risk',p.deviations.Age,'5 if >60'],['BMI_risk',p.deviations.BMI,'5 if out of range'],
                      ['Anemia',p.deviations.Hgb,'5 if Hgb<10'],['Hyd_def',p.deviations.Hyd,'max(0,60−hyd)']
                    ].map(([label,val,formula])=>(
                      <div key={label} style={{ display:'flex', justifyContent:'space-between', fontSize:9, padding:'2px 0' }}>
                        <span style={{ color:C.muted }}>{label}</span>
                        <span style={{ fontFamily:mono, color:p.normalised[label.split('_')[0]]>0.6?C.red:p.normalised[label.split('_')[0]]>0.3?C.yellow:C.green }}>
                  ...
                  <div style={{ background:'#050d18', borderRadius:6, padding:12 }}>
                    ...
                    <div style={{ marginTop:8 }}>
                      <div style={{ fontSize:8, color:C.muted, marginBottom:3 }}>VITAL DEVIATION INDEX</div>
                      <span style={{ fontFamily:mono, fontSize:16, fontWeight:700, color:heatColor(p.score) }}>{(p.vdi*100).toFixed(1)}%</span>
                    </div>
                  </div>
                ...
                <div style={{ marginTop:10 }}>
                  ...
                  {logO && (
                    <div style={{ marginTop:6, background:'#020609', borderRadius:6, padding:12,
                      fontFamily:mono, fontSize:9, border:`1px solid ${C.border}`, lineHeight:1.8 }}>
                      {p.log.map((line,i)=>(
                        <div key={i} style={{ color:i===p.log.length-1?C.green:C.accent }}>&gt; {line}</div>
                      ))}
                    </div>
                  )}
                </div>
