...
export default function HeatmapTab({ processed }) {
  return (
    <div style={{ animation:'fadeIn .4s ease' }}>
      <div style={{ fontSize:8, color:C.muted, letterSpacing:2, marginBottom:16, fontFamily:mono }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:18, flexWrap:'wrap' }}>
        ...
        {[['0–25','#00bcd4','Low'],['25–40','#69f0ae','Guarded'],['40–55','#ffc400','Moderate'],
          ['55–70','#ff9100','Elevated'],['70–85','#ff5722','High'],['85–100','#ff1744','Critical']].map(([r,c,l])=>(
          ...
          {processed.map(p => {
            ...
            return (
              <div key={p.patient_id} style={{
                background: heat+'18',
                border: `1px solid ${heat}55`,
                borderRadius: 8,
                padding: '12px 10px',
                position: 'relative',
                cursor: 'default',
                transition: 'transform .15s',
                boxShadow: p.severity === 'Critical' ? `0 0 14px ${heat}40, inset 0 0 20px ${heat}10` : 'none',
              }}
              ...
              onMouseLeave={e=>e.currentTarget.style.transform='scale(1)'}>
                ...
                <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:6 }}>
                  <div style={{ width:38, height:38, borderRadius:'50%',
                    border:`2px solid ${heat}`, background:heat+'14',
                ...
                <div style={{ marginBottom:4 }}>
                  ...
                  <div style={{ height:3, background:'#0a1830', borderRadius:2 }}>
                    <div style={{ width:`${p.vdi*100}%`, height:'100%', borderRadius:2,
                      background:heat, boxShadow:`0 0 5px ${heat}66` }} />
                  </div>
                </div>
                {/* Top 2 deviating vitals */}
                <div style={{ fontSize:7, color:C.muted, marginTop:4 }}>
                  {Object.entries(p.normalised)
                    .sort(([,a],[,b])=>b-a)
                    .slice(0,2)
                    .map(([k,v])=>(
                      <span key={k} style={{ marginRight:4, color:v>0.6?C.red:v>0.35?C.yellow:C.green }}>
                        {k}:{(v*100).toFixed(0)}%
                      </span>
                    ))}
                </div>
          ...
          <table style={{ width:'100%', borderCollapse:'collapse', fontFamily:mono, fontSize:9 }}>
            ...
            <tbody>
              {processed.map(p => {
                const sc = severityColor(p.severity);
                return (
                  <tr key={p.patient_id} style={{ borderBottom:`1px solid ${C.border}12` }}>
                    ...
                    {VDI_KEYS.map(k => {
                      ...
                      return (
                        <td key={k} style={{ padding:'3px 4px', textAlign:'center' }}>
                          <div style={{ width:32, height:20, background:cell+'22', border:`1px solid ${cell}44`,
                    ...
                    <td style={{ padding:'3px 4px', textAlign:'center' }}>
                      <div style={{ width:36, height:20, background:heatColor(p.score)+'22',
                        border:`1px solid ${heatColor(p.score)}55`, borderRadius:3,
